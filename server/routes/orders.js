...
router.route('/')
	.get((req, res, next) => {
		...
		const selectOrders = 'SELECT O.TICKET_ID, O.ORDER_ID, O.DATE_ORDERED, O.TOTAL_PRICE, O.DELIVERY_ETA, T.TITLE, T.CREATED_AT ' +
...
router.route('/:id')
	.get((req, res, next) => {
		...
		const selectOrder = 'SELECT O.DATE_ORDERED, O.DELIVERY_ETA, T.TITLE, O.TOTAL_PRICE, ' +
